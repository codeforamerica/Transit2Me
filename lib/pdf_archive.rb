...
get '/routeit' do
  if(params['address'])
    ...
    day = event.dateof.split('/')[0].to_f()
    month = event.dateof.split('/')[1].to_f()
    year = event.dateof.split('/')[2].to_f()
    hour = event.timeof.split(':')[0].to_f()
    minute = event.timeof.split(':')[1].to_f()
    return day + "--" + month
